...
public class IntelligentItemLoreAnimator {
    ...
    public static class Builder {
         ...
         */
        public Builder copy(@NotNull IntelligentItemLoreAnimator preset) {
            this.preset = preset;
            return this;
        }
         ...
         * @return The Builder to perform further editing
         */
        public Builder lore(@Nonnegative int index, @NotNull String frame) {
        ...
        public Builder item(@NotNull IntelligentItem intelligentItem) {
            ...
            List<String> lore = itemStack.hasItemMeta() ? itemStack.getLore() : new ArrayList<>();
         ...
         */
        public Builder colors(@NotNull List<Character> frames, IntelligentItemColor @NotNull ... color) throws IllegalArgumentException {
         ...
         */
        public IntelligentItemLoreAnimator build(@NotNull InventoryContents contents) throws IllegalArgumentException, NullPointerException {
            if (this.preset != null) {
     ...
     */
    public void animate(@NotNull JavaPlugin plugin) {
        this.inventory.addLoreAnimator(this);
        animateByType(plugin);
    }

    private void animateByType(@NotNull JavaPlugin plugin) {
    ...
    private void animateWithFlash(@NotNull JavaPlugin plugin) {
        for (Map.Entry<Integer, String> entry : this.loreData.entrySet()) {
            BukkitTask task;
    ...
    private void animateByFullWord(@NotNull JavaPlugin plugin) {
        for (Map.Entry<Integer, String> entry : this.loreData.entrySet()) {
            BukkitTask task;
            task = Bukkit.getScheduler().runTaskTimer(plugin, new Runnable() {
                final HashMap<Integer, String> framesCopy = loreData;
    ...
    private void animateWordByWord(@NotNull JavaPlugin plugin) {
        for (Map.Entry<Integer, String> entry : this.loreData.entrySet()) {
            BukkitTask task;
            task = Bukkit.getScheduler().runTaskTimer(plugin, new Runnable() {
                final HashMap<Integer, String> framesCopy = loreData;
                ...
                public void run() {
                    ...
                    char[] letters = !this.currentLore.isBlank() ? ChatColor.stripColor(lore.get(loreIndex)).toCharArray() : savedLore.toCharArray();
